...
# This code is in R and not in R Markdown because it is too large for compilation

# The functions in this code were divided among Kaio Torres, João Andrade, and Guilherme Cota
# When combining the functions, the three of them worked to identify the errors

# This code is divided into 33 auxiliary functions used to estimate the model
# There are two main functions that estimate the model for 1 (a2Estimate) and 2 (a3Estimate) cities
# There is also a code (a0Master) that calls all 35 functions and performs all the estimations
# The latter takes approximately 55 hours to run
...
# This function loads the data to estimate the model  with 1 metro area.
...
# This function estimates the model with 1 metro area. 
# The code estimates the model using 3 observed households types (defined by 
# the k means analysis), 5 unobserved household types, 2 time periods.
# We estimate: 4*5 = 20 preference parameters, Nh prices (where Nh is the size 
# (14) of the grid of qualities we choose) = 14 parameters, 5*3=15 probabilities
# that map observed and unobserved types, 1 supply elasticity parameter.
# Prices in the first period in Chicago are obtained through normalization. 
# Total number of parameters estimated: 50
# The objective function minimizes moments that measure the difference between
# quality supply and demand, the difference between the implied and the
# observed aggregate rent distributions, and the difference between 
# observed and estimate income and rent correlations.
# Following proposition 2, we normalize housing quality by setting h = v1(h) 
# (i.e. the pricing function is linear v(h)=h) in the reference city (Chicago).
...
a2Estimate<-function(YI,VI,PI,Pop, Corr){
  ...
  # Parameters of observed income distributions by observed types
  # These come from fitting the income distributions using a GLN4
  # distribution. The fit is perfect as these distributions are very
  # flexible. This is equivalent to calculating the empirical CDF from the
  # micro data
  px<-list()
  pxx<-list()
  pxxx<-list()
  ...
  #Parameters of observed aggregate rent distributions by type
  px$v<-PI$GLN4ui$v
  pxx$v<-PI$GLN4uii$v
  ...
  # Probabilities for unobserved types given observed types: Prxa_ib is the
  # probability of someone in observed type x=a to be of the unobserved type i=b
  ...
  # Test initial values of probabilities
  ...
  # As initial values, we use the preference parameters, for all types, that are consistent 
  # with the unconstrained price and income distributions fitting using a GLN4 
  p0i<- PI$GLN4ui
  ...
  # Perturb them slightly in their initial values, which improves the
  ...
  # For equilibrium prices we choose the values of the qualities in hg 
  # as initial values, which are themselves determined by the deflated rent 
  # bins provided in the data
  v01 <- log(hg)
  v02 <- log(hg)

  # Test the initial values satisfy regularity conditions and structural
  # constraints
  ...
  # Guess matching the article's outcome
  ...
  # Change the parameter scale to make value changes more evident when
  # estimating.
  ...
  Q<-function(p_hat){
    ...
    # Recover initial values and remove normalization
    # type = 1 
    upi<-list()
    upi$alpha <- p_hat[1]
    upi$phi <- p_hat[2]*100
    ...
    # Check regularity and structural constraints
    ...
    if (all(cons1>0) & all(cons2>0) & all(cons3>0) & all(cons4>0) & all(cons5>0) & upi$alpha>0 & upii$alpha>0 &
      ...
      # Calculate Supplies and Demand for each quality level
      ...
      if(cond1 & cond2){
        ...
        # Using the rents, check:
        # 1. Market equilibrium in the second period
        eq <- 10*(AgHdt-Rh2)
        eqs <- sum(eq**2)
        eqslow <- sum(eq[1:7]**2)
        eqsup <- sum(eq[8:length(eq)]**2)
        
        # 2. Fit of rents in t=1
        ...
        # 3. Match correlations of rent and income for observed types x implied
  ...
  ## Minimization
  opts<-list(abstol=1e-4,reltol=1e-4,maxit=1000) # maxit was 30,000
  ...
  # minimum quality so that there will be housing consumption
  ...
  # utility of min observed quality 
...
# This function estimates the model with 2 metro areas. 
# The code estimates the model using 3 observed households types (defined by 
# the k means analysis), 5 unobserved household types, 2 time periods.
# We estimate: 4*5 = 20 preference parameters, Nh*3 prices (where Nh is the size (14) of the grid of qualities we
# choose) 14*3 = 42 parameters, 5*3=15 probabilities that map observed and unobserved types, 
# 1 supply elasticity parameter, . Prices in the first period in Chicago are obtained through normalization. 
# Total number of parameters estimated: 78
# The objective function minimizes moments that measure the difference between quality supply and demand, the difference
# between the implied and the observed aggregate rent distributions, and the difference between 
# observed and estimate income and rent correlations.

# Following proposition 2, we normalize housing quality by setting h =
# v1(h) (i.e. the pricing function is linear v(h)=h) in the reference city (Chicago).
...
a3Estimate<-function(YI,VI,PI,Pop,YIm,VIm,PIm,Popm, Corr){
  ...
  # As initial values, we use the preference parameters, for all types, that are consistent 
  # with the unconstrained price and income distributions for Chicago in the first period, fitting using a GLN4 
  p0i<- PI$GLN4ui
  u1<-CalLinearPref(p0i$y,p0i$v)
  u1$kappa<-  0
  u2<- u1
  ...
  # Test the initial values satisfy regularity conditions and structural constraints
  testyi <- Ycutoff(hg,u1,exp(v01))
  testyii <- Ycutoff(hg,u2,exp(v01))
  testyiii <- Ycutoff(hg,u3,exp(v01))
  testyiv <- Ycutoff(hg,u4,exp(v01))
  ...
  #Change the parameter scale to make value changes more evident when estimating.
  u01 <- c(u1$alpha, (u1$phi)/100, (u1$eta)/1000, u1$gamma*10, (u1$kappa)/1000)
  u02 <- c(u2$alpha, (u2$phi)/100, (u2$eta)/1000, u2$gamma*10, (u2$kappa)/1000)
  u03 <- c(u3$alpha, (u3$phi)/100, (u3$eta)/1000, u3$gamma*10, (u3$kappa)/1000)
  u04 <- c(u4$alpha, (u4$phi)/100, (u4$eta)/1000, u4$gamma*10, (u4$kappa)/1000)
  ...
  Q<-function(p_hat){
    ...
    if (all(cons1>0) & all(cons2>0) & all(cons3>0) & all(cons4>0) & all(cons5>0) & upi$alpha>0 & upii$alpha>0 &
      ...
      if(cond1 & cond2 & cond1m & cond2m){
        ...
        # Using the rents, check:
        # 1a. Market equilibrium in the first period for m reference metro
        eq1m <- 10*(AgHdtm-Rhm)
        eq1ms <- sum(eq1m**2)
        eq1mslow <- sum(eq1m[1:7]**2)
        eq1msup <- sum(eq1m[8:length(eq1m)]**2)
        
        # 1b. Market equilibrium in the second period for base and
        # m reference metro
        eq2m <- 10*(AgHdtm-Rh2m)
        eq2ms <- sum(eq2m**2)
        ...
        # 3. Match correlations of rent and income for observed types x
        CorrMoment <- Yvcorrelations(sx, sxx, sxxx, p1, p2, p3, hg, upi,upii,upiii,upiv,upv, vp1, vp2, Corr)
...
CalcCV <- function(y,uph,v,vm,hg){
  ...
  # What is the quality h consumed at income y at prices v in quality grid hg
  # in base metro area
  ...
  # What is the quality h consumed at income y at prices vm in quality grid hg
  # in reference m metro area (after moving)
  ...
  # Calculate the income that gives a household the same utility after moving
  # to the base area that they had in the reference m metro area
  CV <- function(yp){
...
# This function evaluates the GLN4 cdf distribution.
...
# Derivative dy_dh from the FOCs
...
# Derivative dp_dh from the FOCs, when pricing function is linear
...
# This function estimates the quantile of the GLN4 distribution.
...
# A function of the model that determines rents based on the GLN4 distribution
  ...
  if (cond1 ==1){
    ...
    if  (monov) {
      ...
      if (cond2 ==1){
        ...
        # linear function for price of first periodvp1 <- t(h)
    ...
    if (cond1m ==1){
      ...
      if  (monov2){
        ...
        if (cond2m == 1){
          ...
          # linear function for price of first period in the refence metro
          vbase <- t(h)
          vref <- vp2m
          basesupply <- Rh
...
# Calculate income quality consumed elasticity for given type
...
# Calculate income quality consumed elasticity for given type when a linear
# pricing function is assumed
...
# A function that determines the FOC when
# a linear pricing function is assumed
...
#Calculate price quality consumed elasticity for given type when 
# a linear pricing function is assumed
...
#This function calculates the quality distribution when we assume that pricing
# function is linear in the first period.
...
utilconpositive <- function(up, hg) {
  ...
  c <- 1 - up$phi * ((hg + up$eta)**up$gamma) - 0.001 
...
# Function that calculates the quality 'h' from the rent using
# parameters from the utility function
...
# function that calculates the quality 'h' from the rent and parameters
# of the GLN4 distribution and utility.
...
# Calculates the income cutoffs that make the household indifferent between
# quality j and j+1 for all qualities j=1,...J.
...
# Function that determines the quantiles of the income cutoff distribution
...
Yvcorrelations<- function(sx, sxx, sxxx, p1, p2, p3, h, upi,upii,upiii,upiv,upv, vp1, vp2, Corr){
  ...
  # I can calculate the implied joint probability of each one of the
  # combinations for unobserved type i and observed type k
  s<-c(sx,sxx,sxxx)
  sdiag<-diag(s)
  ...
  # Calculate the cumulative prob of this joint probability place in a vector
...
#### This program estimates the housing model

## 1. Load the data for the 1 and 2 metro area model estimation

# setwd('')

WP <- readRDS('MetroAreasData.rds')

## 2. Estimate the 1 metro area model with 5 unobserved types

a2 <- a2Estimate(WP$Chi$YIk,WP$Chi$VIk,WP$Chi$PIk,WP$Chi$Pop,WP$Chi$Corr)
saveRDS(a2, file = "a2_maxit1000.rds")

## 3. Estimate the 2 metro area2 model with 5 unobserved types
a3 <- a3Estimate(WP$Chi$YIk,WP$Chi$VIk,WP$Chi$PIk,WP$Chi$Pop,WP$NYC$YIk,WP$NYC$VIk,WP$NYC$PIk,WP$NYC$Pop,WP$Agr$Corr)
saveRDS(a3, file = "a3_maxit1000.rds")
